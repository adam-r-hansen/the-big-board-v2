...
export default function WrinkleCard({ wrinkle, pick, onSelectTeam, disabled }: Props) {
        ...
        <Box sx={{ p: 2 }}>
          {loadingGames ? (
            ...
            <>
              ...
              {/* ✅ Replaces MUI Grid/Grid2 with a plain CSS grid (works across MUI versions) */}
              <Box
                sx={{
                  display: 'grid',
                  gap: 1,
                  gridTemplateColumns: {
                    xs: 'repeat(2, minmax(0, 1fr))', // was xs={6}
                    sm: 'repeat(3, minmax(0, 1fr))', // was sm={4}
                    md: 'repeat(4, minmax(0, 1fr))', // was md={3}
                  },
                }}
              >
                {oofGames
  ...
  const TeamRow = ({ team, isHome }: { team: Team; isHome: boolean }) => {
    ...
    const teamSpread =
      wrinkle.spread_team_id === team.id ? wrinkle.spread : wrinkle.spread ? -wrinkle.spread : null;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Chip label={getKindLabel(wrinkle.kind)} size="small" sx={{ bgcolor: 'rgba(255,255,255,0.2)', color: 'white', height: 22 }} />
      </Box>
